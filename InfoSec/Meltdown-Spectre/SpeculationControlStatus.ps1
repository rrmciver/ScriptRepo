...
# Source: https://gallery.technet.microsoft.com/scriptcenter/Speculation-Control-e36f0050
#
# Extracted Get-SpeculationControlSettings from the provided module for easy execution accross Windows versions.
# Added lines to call the Get-SpeculationControlSettings functions and write the output to the registry.
# Can then use SCCM Compliance Settings to check the registry values to determine system compliance.
...
$RegPath = "HKLM:\Software\Security\SpeculationControl"
...
$SpecCtrlStatus = Get-SpeculationControlSettings | Out-String
$SpecCtrlStatus = $SpecCtrlStatus.Split([Environment]::NewLine)
$SpecCtrlStatus = $SpecCtrlStatus | ? {$_}
ForEach ($line in $SpecCtrlStatus)
{
    $SplitLine = $line.Split(":")
    $RegName = $SplitLine[0].Trim()
    $RegValue = $SplitLine[1].Trim()
